...
{{? it.sources.length > 0 }}
    {{= window.tpl.search({ label: 'Search source files', classes: 'js-m-sources__search'}) }}
    <ul class="m-sources__list js-m-sources__list">
        {{~ it.sources :source }}
            {{= window.tpl.sourcesitem(source) }}
        {{~}}
    </ul>
    <div class="o-settings__blank js-m-sources__no-results m-sources__no-results">
